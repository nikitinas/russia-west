...
const SECTION_HEADINGS: Record<Locale, { russia: string; west: string; summary: string }> = {
  ru: {
    russia: "Русская онтология: Человек → Природа → Общество",
    west: "Западная онтология: Человек → Общество → Природа",
    summary: "Сводная таблица",
  },
  en: {
    russia: "Russian ontology: Human → Nature → Society",
    west: "Western ontology: Human → Society → Nature",
...
export type OntologyByPerspective = {
  intro: string;
  russiaBlock: string;
  westBlock: string;
  summary: string;
...
/**
 * Splits ontology markdown into intro, Russian block, Western block, and summary
 * so the UI can show one perspective at a time.
 */
export function splitOntologyByPerspective(content: string, locale: Locale): OntologyByPerspective {
